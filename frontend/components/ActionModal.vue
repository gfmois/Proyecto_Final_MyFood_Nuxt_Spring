<script setup>
const { isModalVisible, itemSelected, title, hasTotal } = defineProps({
    ...
    <div v-if="isModalVisible">
        ...
        <div class="fixed top-0 left-0 p-8 w-screen h-screen z-[51]">
            <div class="bg-gray-100 rounded-lg p-4 w-full h-full relative">
                ...
                <div :class="`grid grid-cols- md:grid-cols-2 gap-3 overflow-y-scroll box-border ${hasTotal ? 'h-[80%]' : 'h-[90%]'}`">
                    <slot />
                </div>
                <div class="bg-crimson-600 h-20 w-full absolute bottom-5 p-4 left-0 text-white flex items-center justify-end" v-if="hasTotal">
                    <div class="flex items-center justify-center gap-2 p-2 text-lg mr-20">
                        <h1>Total: </h1>
                        <p>20</p>
                    </div>
                </div>
