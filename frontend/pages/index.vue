...
<template>
    <div>
        ...
        <div
            ...
            <div
                class="absolute xs:w-3/4 xs:h-2/5 sm:h-2/4 lg:w-1/3 lg:h-2/4 z-50 top-20 xs:left-12 lg:left-20 bg-white/80 p-6 text-black rounded-lg shadow-lg font-mono text-center">
                ...
                <div class="w-full h-1/2 flex items-start justify-center">
                    <div class="w-1/2 p-4 flex flex-col gap-2 border border-gray-500/60 mt-4">
                        <select name="city" v-model="obj.value.city"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option disabled selected value="">Selecciona una ciudad</option>
                            <option v-bind:key="city" :value="city" v-for="city in selectInfo.cities">{{ city }}
                            </option>
                        </select>

                        <select name="cateogory" v-model="obj.value.category"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5">
                            <option disabled selected>Selecciona una categoria</option>
                            <option v-bind:key="category" calue="category" v-for="category in selectInfo.categories">{{ category }}</option>
                        </select>
                    </div>
                </div>
            ...
            <div class="flex items-center flex-wrap justify-center gap-2 h-full w-full xs:flex-col sm:flex-row">
