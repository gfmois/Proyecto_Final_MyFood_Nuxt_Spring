            ...
            <div class="p-4 mb-4 w-full h-full divide-y divide-y-reverse border-t-2 border-b-2">
                <div class="flex w-full h-full flex-wrap items-end justify-center">
                    <div class="flex-1 items-center justify-center flex cursor-pointer h-full w-full"
                        v-on:click="() => { seeFilters = !seeFilters; closeOthers(-1) }">{{ seeFilters ? "Mostrar" :
                            "Esconder" }} Filtros</div>
                </div>
            </div>
            <div class="bg-gray-200 mb-3 w-full h-full p-2 flex flex-col sm:flex-row gap-3 items-center"
                v-if="!seeFilters">
                ...
                <LayoutFilterBean filterName="Ciudad" :filterOptions="info.cities" @closeOthers="$e => closeOthers(0)"
                ...
                <LayoutFilterBean filterName="Categoria" :filterOptions="info.categories"
                    @closeOthers="$e => closeOthers(1)" @value="$e => filters.value.category = $e"
                    :parentCloses="filtersCloses.value[1]" />
                <!-- FIXME: Change this to line -->
                <LayoutFilterBean filterName="Precio" :filterOptions="['5', '10', '15', '20']"
                    @closeOthers="$e => closeOthers(2)" @value="$e => filters.value.price = $e"
                    :parentCloses="filtersCloses.value[2]" />
            </div>
            <div class="flex gap-4 flex-col">
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4"
                    v-if="restaurants.length > 0">
                    <NuxtLink
                        class="max-w-2xl mx-auto bg-white rounded-xl shadow-md overflow-hidden cursor-pointer hover:shadow-2xl"
