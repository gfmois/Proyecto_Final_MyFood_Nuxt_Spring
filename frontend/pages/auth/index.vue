		...
		<main>
			<div class="book" @mouseover="book_open = true" @mouseleave="book_open = false, login = true, register = false">
				...
				<div class="book-content"
					:class="{ 'book-content-hover': register && book_open, 'index_up': book_open && login == true }">
					<div class="login">
						<div class="title">
							<h1>{{ $t('auth.login') }}</h1>
						...
						<div class="change-register">
							...
							<button @click="register = true, login = false, isLogin = true">{{ $t('auth.register')
							}}</button>
						</div>
					</div>
				</div>
...
.login .inputs .multiple-inputs,
.register .inputs .multiple-inputs {
	width: 100%;
}
